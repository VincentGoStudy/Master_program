...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # 计算Q, K, V
        query = self.query_conv(x).view(batch_size, -1, depth * height * width)  # [B, C//8, D*H*W]
        key = self.key_conv(x).view(batch_size, -1, depth * height * width)  # [B, C//8, D*H*W]
        value = self.value_conv(x).view(batch_size, -1, depth * height * width)  # [B, C, D*H*W]

        # 计算注意力权重
        attention = torch.bmm(query.permute(0, 2, 1), key)  # [B, D*H*W, D*H*W]
        attention = F.softmax(attention, dim=-1)  # 对最后一个维度进行 softmax

        # 计算注意力输出
        out = torch.bmm(value, attention.permute(0, 2, 1))  # [B, C, D*H*W]
        out = out.view(batch_size, C, depth, height, width)  # 恢复原始形状

        return self.gamma * out + x  # 残差连接


class CustomResNet34_3D(nn.Module):
    def __init__(self, num_classes):
        super(CustomResNet34_3D, self).__init__()
        self.num_classes = num_classes

        # 使用 MONAI 提供的 3D ResNet34
        self.base = resnet34(spatial_dims=3, n_input_channels=1, num_classes=num_classes)

        # 自注意力层，输入通道数与 ResNet 第一层输出通道数一致（通常是 64）
        self.self_attention = SelfAttention(in_channels=64)
    ...
    def forward(self, x):
        ...
        x = self.base.bn1(x)  # 批归一化
        x = torch.nn.functional.relu(x)  # ReLU 激活
...
model = CustomResNet34_3D(num_classes=6).to(device)
